...
export default function SuaLoaiBienBao() {
    const [bienbao, setLoaiBienBao] = useState([])
    const [thongbao, setThongBao] = useState({ show: false, message: "", success: false })
   
    const handleInputChange = (e) => {
        setLoaiBienBao({ ...bienbao, [e.target.name]: e.target.value }) //... để rải hết dữ liệu trong câu hỏi
    }
    const handleSubmit = async (e) => {
        e.preventDefault()
        const response = await axios.put(process.env.REACT_APP_API_HOST + "/a1/sualoaibienbao", bienbao)
        setThongBao(() => {
            if (response) {
    ...
    useEffect(() => {
        const getDataName = async () => {
            ...
            const response = await axios.get(process.env.REACT_APP_API_HOST + "/a1/1loaibienbao", { params: { id: id } })
            console.log(response);
            setLoaiBienBao(response.data.result[0])
        }
        getDataName()
    }, [])
    return (
            ...
            <Container>
                <Form onSubmit={handleSubmit}>
                    <Form.Group controlId="form.Name" className="ib">
                        ...
                        <Form.Control className="inbo" type="text" name="TenLoaiBB" value={bienbao.TenLoaiBB} onChange={handleInputChange} />
                    </Form.Group>
